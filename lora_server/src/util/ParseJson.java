...
public class ParseJson {
	...
	public static HashMap<String,Info> parseOfJson(String jsonstr) {
	...
	/*		@SuppressWarnings("unchecked")
			...
			InfoLoraModEnd loraendpkt = new InfoLoraModEnd();
			InfoFSKModEnd fskendpkt = new InfoFSKModEnd();
			InfoGateWayStat gwstat = new InfoGateWayStat();
			...
			if(!json.isNull("rxpk")){
				...
				for(int i = 0; i < rxpk_arry.length(); i ++){
					...
					//	loraendpkt.time = rxpk_arry.getJSONObject(i).getString("time");
						...
						loraendpkt.data = rxpk_arry.getJSONObject(i).getString("data");
						InfoMap.put("Info.InfoLoraModEnd" + i, loraendpkt);
						...
						System.out.println("data:" + loraendpkt.data);
					} else {
					//	fskendpkt.time = rxpk_arry.getJSONObject(i).getString("time");
						...
						fskendpkt.data = rxpk_arry.getJSONObject(i).getString("data");
						InfoMap.put("Info.InfoFSKModEnd" + i, fskendpkt);
						System.out.println("--------------" + i + "---------------");
						...
						System.out.println("data:" + fskendpkt.data);
					}
				}
			} if(!json.isNull("stat") ){					
			//stat_________________________________________________________
				...
				InfoMap.put("Info.InfoGateWayStat", gwstat);
				...
				if(!stat.isNull("error") )	{
					...
					gwstat.time = stat.getString("time");
					gwstat.txnb = stat.getInt("txnb");
					InfoMap.put("Info.InfoPktError", gwstat);
				}
			}
			return InfoMap;
	...
	public static void main(String[] args){
		...
		HashMap<String, Info> InfoMap = pjs.parseOfJson(pjs.json);
		Info info; 
